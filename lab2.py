...
#     name: str = "get_state"
#     description: str = (
#         "Get full world snapshot: all robot positions/headings, all objects, and world metadata."
#     )
...
class SendMessageTool(BaseTool):
    """Post a message to the peer agent's /inbox endpoint."""

    name: str = "send_message"
    description: str = (
        "Send a text message to the other robot agent. "
        "Use this to share observations, coordinate actions, or ask for help."
    )
    args_schema: Type[_SendMessageInput] = _SendMessageInput

    connector: HTTPConnector
    robot_id: str
    peer_robot_id: str
    peer_url: str  # full URL, e.g. "http://localhost:9002/inbox"
    ...
    def _run(self, message: str) -> str:
        msg = HTTPMessage(
            method="POST",
            payload={"from": self.robot_id, "message": message},
        )
        resp = self.connector.service_call(msg, self.peer_url, timeout_sec=10.0)
        try:
            result = json.loads(resp.payload)
        except Exception:
            result = resp.payload
        # Record the outgoing message so get_chat_history shows the full conversation
        self.chat_history.append(self.robot_id, self.peer_robot_id, message)
        return f"Message sent. Peer response: {result}"


class GetChatHistoryTool(BaseTool):
    """Read the accumulated message log between both agents."""

    name: str = "get_chat_history"
    description: str = (
        "Read the full message history exchanged between you and the other robot agent."
    )
    args_schema: Type[_NoArgs] = _NoArgs
    ...
    def _run(self) -> str:
        msgs = self.chat_history.get_all()
        if not msgs:
            return "No messages exchanged yet."
        return "\n".join(
            f"[{m['from']} → {m['to']}]: {m['message']}" for m in msgs
        )
...
def _build_tools(
    ...
    return [
        GetOwnStateTool(**api_kwargs),
        GetVisibleObjectsTool(**api_kwargs),
        GoToPositionTool(**api_kwargs),
        RotateTool(**api_kwargs),
        GrabTool(**api_kwargs),
        ReleaseTool(**api_kwargs),
        GetCameraTool(**api_kwargs),
        SendMessageTool(
            connector=connector,
            robot_id=robot_id,
